// 模拟地址验证结果函数
function mockStandardizeAndValidateAddress(withParcelMatch) {
  const parcel_match = {
    "features": [
      {
        "geometry": {
  ...
  return Promise.resolve({
    ...
    "parcelCentroid": null,
    "parcelDetails": null,
    "parcel_match": withParcelMatch && parcel_match,
...
function clearSearchResults() {
  // 移除所有搜索结果图层
  if (window.searchResultLayers && window.searchResultLayers.length > 0) {
    window.searchResultLayers.forEach(layer => {
      if (window.map) {
    ...
    // 清空数组
    window.searchResultLayers = [];
  }
  
  // 清除显示的信息
  document.getElementById('beforeAddress').textContent = '';
  document.getElementById('afterAddress').textContent = '';
  document.getElementById('confidenceScore').textContent = '';
  document.getElementById('summary').textContent = '';
  document.getElementById('parcelDetails').textContent = '';
  
  // 清除任何标签内容
  const summaryLists = document.querySelectorAll('[id^="summaryList"]');
  summaryLists.forEach(list => {
    list.innerHTML = '';
  });
  
  // 重置标签以显示计数
  const tabs = document.querySelectorAll('.tabs li[data-tab^="tab"]');
  tabs.forEach((tab, index) => {
    if (window.olLayers && index < window.olLayers.length) {
      tab.textContent = window.olLayers[index].title;
    }
  });
  
  console.log('已清除所有搜索结果');
}